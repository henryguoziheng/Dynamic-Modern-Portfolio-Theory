...
#timeList = [1,2,3,4,5]
covList = np.arange(-0.005, 0.005, 0.001)
MeanVar_Dynamic = []
MeanVar_Sbs = []
for cov in covList:

    covMat = np.array([[0.008649, cov],
                       [cov,    0.011025]])  # covariance matrix
...
plt.scatter(covList, MeanVar_Dynamic, label='dynamic')
plt.scatter(covList, MeanVar_Sbs, label='step by step')
plt.title('Dynamic Vs Step by step varying covariance')
plt.xlabel('Covariance')
plt.ylabel('MeanVar at T=2')
plt.legend(['dynamic', 'step by step'])
#plt.savefig('Dynamic Vs Step by step varying covariance')
plt.show()
