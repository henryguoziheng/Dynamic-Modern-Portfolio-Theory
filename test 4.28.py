...
for horizon in timeList:
    ...
    p_list = np.array([[0.4, 0.5, 0.2, 0.55, 0.5, 0.7, 0.45, 0.5, 0.7, 0.45],
                       # Probability for stock moving down or up for each step. Columns sum to 1.
                       [0.6, 0.5, 0.8, 0.45, 0.5, 0.3, 0.55, 0.5, 0.3, 0.55]])
    r_list = np.array([[-0.08, -0.12, -0.09, -0.07, -0.06, -0.13, -0.05, -0.08, -0.05, -0.06],
                       # Stock return at each time step corresponding to the probability matrix.
    ...
    def W(phi, t):  # input phi is a (2^(t+1)-1) array
        ...
        if t >= 1:
            x1 = W(phi, t - 1)
            y1 = (1 + (1 - phi[power(2, t - 1) - 1:power(2, t) - 1]) * r + phi[power(2, t - 1) - 1:power(2, t) - 1] *
                  r_list[0, t - 1])
            x2 = W(phi, t - 1)
            y2 = (1 + (1 - phi[power(2, t - 1) - 1:power(2, t) - 1]) * r + phi[power(2, t - 1) - 1:power(2, t) - 1] *
                  r_list[1, t - 1])
    ...
    res = minimize(MeanVar, phi, args=(horizon), method='SLSQP', constraints={'type': 'ineq', 'fun': lambda x: x},
                   options={'xtol': 1e-8, 'disp': False})  # With constraint of no short selling
